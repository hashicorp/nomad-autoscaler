...
func (n *IBMCloudIG) Scale(action sdk.ScalingAction, config map[string]string) error {
	...
	if uerr != nil {
		return fmt.Errorf("IG: %s: Exception in UpdateInstanceGroup: %v\n", instance_group_id, uerr)
	}
	...
	n.logger.Info(
		fmt.Sprintf(
			...
			instance_group_id, *ig.Name, action.Count,
		),
	)
...
func (n *IBMCloudIG) Status(config map[string]string) (*sdk.TargetStatus, error) {
	...
	if n.vpc == nil {
		...
		if verr != nil {
			return nil, fmt.Errorf("Exception in NewVpcV1UsingExternalConfig: %v\n", verr)
		}
		n.vpc = vpc
	}
	...
	ig, _, err := n.vpc.GetInstanceGroup(&vpcv1.GetInstanceGroupOptions{ID: &instance_group_id})
	if err != nil {
		return nil, fmt.Errorf("IG: %s: Exception in GetInstanceGroup: %v\n", instance_group_id, err)
	}
