...
* plugin/apm/nomad: Add support for grouping nodes by `datacenter`, `node_pool`, or a combination of multiple pool identifiers in node pool APM queries. Previously only `node_class` was supported. A new combined query format (`key1=val1+key2=val2`) enables filtering by multiple attributes simultaneously. [[GH-1291](https://github.com/hashicorp/nomad-autoscaler/pull/1291)]
