...
type HorizontalAppWorker struct {
	id            string
	logger        hclog.Logger
...
// NewHorizontalAppWorker returns a new HorizontalAppWorker instance.
func NewHorizontalAppWorker(l hclog.Logger, pm *manager.PluginManager, m *policy.Manager, b *Broker) *HorizontalAppWorker {
	id := uuid.Generate()

	return &HorizontalAppWorker{
		id:            id,
		logger:        l.Named("worker").With("id", id),
		pluginManager: pm,
		policyManager: m,
		broker:        b,
	}
...
func (w *HorizontalAppWorker) Run(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
		...
		eval, token, err := w.broker.Dequeue(ctx, policyType)
		if err != nil {
		...
		if err := w.handlePolicy(ctx, eval); err != nil {
			logger.Warn("failed to evaluate policy", "err", err)
...
func (w *HorizontalAppWorker) handlePolicy(ctx context.Context, eval *sdk.ScalingEvaluation) error {
