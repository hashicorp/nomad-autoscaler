...
# KBT_VERSION 1.3.1 Generated by `kt dev earthly`
#
# This file will be regenerated automatically in each Jenkins run, and its source of truth is the `project.yml` file.
# Docs here: https://docs.kntk.cloud/toolchain/kentik-build-tools/earthly_kbt/
#
...
#autoscaler:
#    FROM hashicorp/nomad-autoscaler:0.4.1
#
#    SAVE ARTIFACT /bin/nomad-autoscaler /

#image:
#    FROM $DEFAULT_DOCKER_REGISTRY/$RUN_BASE_IMAGE
#
#    RUN mkdir /opt/autoscaler-plugins
#
#    COPY --keep-ts +autoscaler/nomad-autoscaler /usr/local/bin/
#    COPY --keep-ts +kbt-build/output /opt/autoscaler-plugins/
...
# Edit this step to install extra things in prepare phase for the build image, before all other installs or copying
KBT_PREPARE_IMAGE:
    FUNCTION

# Edit this step to copy only required files, the less you copy the bigger chance to cache hit
KBT_COPY_SOURCE:
    FUNCTION
    COPY --keep-ts --dir . .
...
KBT_RUN_TESTS:
    FUNCTION
    DO --pass-args kbt_lib+GO_WITH_GITHUB --cmd="make test"

KBT_RUN_BUILD:
    FUNCTION
    DO --pass-args kbt_lib+GO_WITH_GITHUB --cmd="unset GOPATH && make all"
...
kbt-prepare:
    ...
    DO --pass-args kbt_lib+KBT_GO_FLAGS --PROJECT_NAME=nomad-autoscaler
...
kbt-push:
    FROM +kbt-image
    DO --pass-args kbt_lib+KBT_PUSH --IMAGE_NAME=kentik-nomad-autoscaler
...
kbt-test-nomad-manifests:
    ...
    COPY --keep-ts --dir project.yml nomad /data/service/ktools/repos/nomad-autoscaler/
    DO --pass-args github.com/kentik/kentik-nomad:main+RUN_INTEGRATION_TESTS
...
kbt-package-nomad-manifests:
    ...
    ARG BUILD_NUMBER=0
    RUN ./package-nomad-manifests.sh nomad-autoscaler $SANITIZED_BRANCH.$BUILD_NUMBER service main
