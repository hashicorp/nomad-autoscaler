...
func NewBaseWorker(l hclog.Logger, pm *manager.PluginManager, m CoolDownEnforcer, b *Broker, queue string) *BaseWorker {
	id := uuid.Generate()
...
func (w *BaseWorker) handlePolicy(ctx context.Context, eval *sdk.ScalingEvaluation) error {
	...
	if currentStatus.Count < eval.Policy.Min {
		...
		return w.scaleTarget(logger, eval.Policy, action, currentStatus)
	}
	...
	err := w.scaleTarget(logger, eval.Policy, *winner.action, currentStatus)
	if err != nil {
		return err
	}
...
func (w *BaseWorker) scaleTarget(
	...
	err := w.runTargetScale(policy, action)
	if err != nil {
	...
	w.cooldownEnforcer.EnforceCooldown(policy.ID, policy.Cooldown)
	return nil
}
...
// functionality.
func (w *BaseWorker) runTargetScale(policy *sdk.ScalingPolicy, action sdk.ScalingAction) error {
	// Trigger a metric measure to track latency of the call.

	// Dispense taget plugin.
	targetPlugin, err := w.pluginManager.Dispense(policy.Target.Name, sdk.PluginTypeTarget)
	if err != nil {
		return fmt.Errorf(`target plugin "%s" not initialized: %v`, policy.Target.Name, err)
	}
	targetInst, ok := targetPlugin.Plugin().(target.Target)
	if !ok {
		return fmt.Errorf(`"%s" is not a target plugin`, policy.Target.Name)
	}

	labels := []metrics.Label{{Name: "plugin_name", Value: policy.Target.Name}, {Name: "policy_id", Value: policy.ID}}
	defer metrics.MeasureSinceWithLabels([]string{"plugin", "target", "scale", "invoke_ms"}, time.Now(), labels)

	return targetInst.Scale(action, policy.Target.Config)
}
