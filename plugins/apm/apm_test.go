...
func TestRPCServerQuery(t *testing.T) {
...
func TestRPCServerQueryMultiple(t *testing.T) {
	client := plugin.NewClient(&plugin.ClientConfig{
		HandshakeConfig: handshake,
		Plugins:         map[string]plugin.Plugin{"apm": &Plugin{}},
		Cmd:             exec.Command("../test/bin/noop-apm"),
		//Cmd: exec.Command("../../bin/plugins/nomad-apm"),
	})
	defer client.Kill()
	...
	apmImpl.SetConfig(map[string]string{})

	result, err := apmImpl.QueryMultiple("fixed:5", r)
	//result, err := apmImpl.QueryMultiple("taskgroup_avg_cpu/cache/example", r)
	require.NoError(t, err)
