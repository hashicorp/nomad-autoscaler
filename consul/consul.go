...
type Consul struct {
	...
	// serviceID is the full ID used to register this agent within Consul. This
	// is a composite of the agentID and serviceName.
	serviceID string
...
func NewConsul(log hclog.Logger, client *api.Client, service string, addr string, port int) (*Consul, error) {
	c := Consul{
		...
		poolDiscoveryReloadCh: make(chan struct{}),
	}
	c.SetLogger(log)
	return &c, nil
}

// SetLogger updates the logger used within Consul ensuring the standard
// name is applied.
func (c *Consul) SetLogger(log hclog.Logger) { c.logger = log.Named("consul") }
