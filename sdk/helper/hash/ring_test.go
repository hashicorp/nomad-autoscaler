...
func TestConsistentRing_Update(t *testing.T) {
	testCases := []struct {
		...
		{
			inputConsistentRing: NewConsistentRing(nil),
			inputUpdateMembers:  generateRingMembers(3),
			name:                "empty ring",
		},
		{
			inputConsistentRing: NewConsistentRing(generateRingMembers(5)),
			inputUpdateMembers:  generateRingMembers(3),
			name:                "already populated ring",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, member := range tc.inputUpdateMembers {
				h := tc.inputConsistentRing.hasher.Hash64(member.Bytes())
