...
const (
	...
	// plugins.
	pluginName = "kentik-nomad-target"
...
func (t *TargetPlugin) SetConfig(config map[string]string) error {
	...
	t.nodeWatcherLock.Lock()
	defer t.nodeWatcherLock.Unlock()

	// Start node watcher loop if not started
	if t.nodeWatcher == nil {
		t.nodeWatcher, err = node.NewNodeStatusWatcher(client, t.logger)
		if err != nil {
			return errors.Wrapf(err, "could not initialize node status watcher")
		}
	} else { // refresh client
		t.nodeWatcher.SetClient(client)
	}
...
func (t *TargetPlugin) Status(config map[string]string) (*sdk.TargetStatus, error) {
	...
	if checkUnknown, ok := config[configCheckUnknownAllocs]; ok && checkUnknown == "true" {
